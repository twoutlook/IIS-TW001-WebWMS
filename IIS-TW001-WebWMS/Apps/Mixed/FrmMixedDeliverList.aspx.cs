...
public partial class Apps_Mixed_FrmMixedDeliverList : WMSBasePage
{
    ...
    {
        int pageCount = 0;
        DataTable dt = query.MixedQueryList(this.txtMixedCode.Text.Trim(), txtERP_No.Text.Trim(), txtPalledCode.Text.Trim(), txtITYPE.SelectedValue, txtCinvcode.Text.Trim(), "1", this.txtCCREATEOWNERCODE.Text.Trim(), txtDCREATETIMEFrom.Text.Trim(), txtDCREATETIMETo.Text.Trim(), CurrendIndex, PageSize, out pageCount);
    ...
    public void InitPage()
    {
        ...
        //本页面打开新增窗口
        //Help.DropDownListDataBind(SysParameterList.GetList("", "", "MixedStatus", false, -1, -1), this.ddlCSTATUS, "全部", "FLAG_NAME", "FLAG_ID", "");
        ...
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
               // e.Row.Cells[5].Text = GetOUTTypeName(e.Row.Cells[5].Text);
                ...
                this.OpenFloatWinMax(linkView, BuildRequestPageURL("FrmMixedDeliverCall.aspx", SYSOperation.View, strKeyID), Resources.Lang.FrmMixedList_Msg04, "Mixed_DeliverCall"); //配料详情
                string status = this.grdMIXED.DataKeys[e.Row.RowIndex].Values[1].ToString();//状态
                if (status == "1") //配料中 //0	配料中  1	暂存中   3	已完成  2	已交付//if (e.Row.Cells[6].Text.Equals("暂存中"))
                {
                    ...
                    //{
                        ...
                        this.OpenFloatWinMax(linkModify, BuildRequestPageURL("FrmMixedDeliverCall.aspx", SYSOperation.Modify, strKeyID), Resources.Lang.FrmMixedList_Msg04, "Mixed_DeliverCall"); //配料详情
                    //}
                }
                //e.Row.Cells[6].Text = query.GetStatusName(e.Row.Cells[6].Text); 从DB直接拿出来
            }
        }
        catch (Exception)
        {
