...
public partial class Apps_BASE_FrmBase_SSX : WMSBasePage
{
    #region 常量
    ...
    public bool CheckData()
    {
        ...
        {
            this.Alert(Resources.Lang.FrmBase_SSX_Msg01); //输送线编号项不允许空！
        ...
        if (this.txtCraneID.Text.Trim().Length > 0)
        {
            ...
            {
                this.Alert(Resources.Lang.FrmBase_SSX_Msg02);//输送线编号项超过指定的长度30！
                this.SetFocus(txtCraneID);
                return false;
            }
        }
        ...
        {
            this.Alert(Resources.Lang.FrmBase_SSX_Msg03); //输送线编号项只允许输入英文字母和数字,特殊符号-_！
            this.SetFocus(txtCraneID);
            return false;
        }
        //
        ...
        {
            this.Alert(Resources.Lang.FrmBase_SSX_Msg04); //输送线名称项不允许空！
        ...
        if (this.txtCraneNAME.Text.Trim().Length > 0)
        {
            ...
            {
                this.Alert(Resources.Lang.FrmBase_SSX_Msg05);//输送线名称项超过指定的长度30！
                this.SetFocus(txtCraneNAME);
                return false;
            }
        }
        ...
        {
            this.Alert(Resources.Lang.FrmBase_SSX_Msg10); //输送线编号项只允许输入数字！
            this.SetFocus(txtCraneID);
            return false;
        }
        return true;
    ...
    public void InitPage()
    {
        ...
        this.btnBack.Attributes["onclick"] = "window.parent.document.getElementById('ctl00_txtPageSizeDetail').value='1';window.parent.document.all['ctl00_ContentPlaceHolderMain_btnSearchSSX'].click(); CloseMySelf('BASE_SSX');return false;";
        this.grdBASE_CraneDetail.DataKeyNames = new string[] { "ID", "FLAG" };
        //本页面打开新增窗口        
        this.btnNew.Attributes["onclick"] = "PopupFloatWinMax('" + WMSBasePage.BuildRequestPageURL("FrmBase_SSX_D.aspx", SYSOperation.New, hdnID.Value) + "','" + Resources.Lang.WMS_Common_Element_NewSite + "','BASE_SSX_D');return false;";//新增站点
        //设置保存按钮的文字及其状态
        ...
        if (this.CheckData())
        {
            ...
            try
            {
                ...
                if (this.Operation() == SYSOperation.Modify)
                {
                    ...
                    {
                        con.Save();
                        this.AlertAndBack("FrmBase_SSX.aspx?" + BuildQueryString(SYSOperation.Modify, strKeyID), Resources.Lang.Common_SuccessSave); //保存成功
                    }
                }
                else if (this.Operation() == SYSOperation.New)
                {
    ...
    protected void grdBASE_CraneDetail_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            string strKeyID = this.grdBASE_CraneDetail.DataKeyNames.Length > 0 ? this.grdBASE_CraneDetail.DataKeys[e.Row.RowIndex].Values[0].ToString() : ""; 
            HyperLink linkModify = (HyperLink)e.Row.Cells[e.Row.Cells.Count - 1].Controls[0];
            linkModify.NavigateUrl = "#";

            this.OpenFloatWinMax(linkModify, BuildRequestPageURL("FrmBase_SSX_D.aspx", SYSOperation.Modify, strKeyID), Resources.Lang.FrmBase_CraneConfigEdit_Msg15, "BASE_SSX_D"); //站点详情
