...
public partial class FrmPANDIANReport : WMSBasePage
{
    ...
    {
        List<V_PANDIANReport> list = new List<V_PANDIANReport>();

        IGenericRepository<V_PANDIANReport> con = new GenericRepository<V_PANDIANReport>(context);
        var caseList = from p in con.Get()
                       orderby p.dcreatetime descending
                       where 1 == 1
                       select p;
        if (txtPDDH.Text != string.Empty)
            caseList = caseList.Where(x => !string.IsNullOrEmpty(x.cticketcode) && x.cticketcode.Contains(txtPDDH.Text.Trim()));
        ...
        if (dplCHECKTYPE.SelectedValue != "")
            caseList = caseList.Where(x => x.checktype.ToString().Equals(dplCHECKTYPE.SelectedValue));
        ...
        btnExcel.ExcelName = Resources.Lang.FrmPANDIANReport_Content1;// "盘点报表";
        ...
        flagList.Add(new Tuple<string, string>("checktype", "CHECKTYPE"));//类型
    ...
    protected void grdINBILL_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        //    e.Row.Cells[3].Text = e.Row.Cells[3].Text == "0" ? "物理" : "循环";
        //}
